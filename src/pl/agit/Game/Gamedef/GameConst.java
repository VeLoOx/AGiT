package pl.agit.Game.Gamedef;

public interface GameConst {
	
	//----CSS
	String CSS_PACKAGE_PATH = "/pl/agit/css/scene.css";
	
	//----SKRYPTY
	String JS_MAINSHIP = "/GameScripts/Ship.js";
	String JS_MAINSHIP_NAME = "/GameScripts/mainship";
	
	String JS_ALIENSHIP = "/GameScripts/AlienShip.js";
	String JS_ALIENSHIP_NAME = "/GameScripts/alienship";
	
	String JS_ASTEROID_DEMOLITION = "/GameScripts/AsteroidDemolition.js";
	String JS_ASTEROID_DEMOLITION_NAME = "/GameScripts/asteroiddemo";
	
	String JS_ALIEN_MAP = "/GameScripts/AlienMaps.js";
	String JS_ALIEN_MAP_NAME = "/GameScripts/alienname";
	
	
	//----GRAFIKI
	String GFX_MAINSHIP = "/GameGfxFiles/ship.png";
	String GFX_MAINSHIP_NAME = "MainShip";
	String GFX_MAINSPACE = "/GameGfxFiles/space.jpg";
	String GFX_MAINSPACE_NAME="MainSpace";
	String GFX_ASTEROID = "/GameGfxFiles/ast.png";
	String GFX_ASTEROID_NAME="Asteroid";
	String GFX_ALIENSHIP1 = "/GameGfxFiles/alienShip2.png";
	String GFX_ALIENSHIP1_NAME="AlienShip1";
	
	
	
	
	//----DZWIEKI
	String A_MAINSHIP_LASER = "/GameAudioFiles/laser.mp3";
	String A_MAINSHIP_LASER_NAME = "laser";

}
